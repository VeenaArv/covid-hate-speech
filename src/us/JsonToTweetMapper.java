...
public class JsonToTweetMapper extends Mapper<LongWritable, Text, TweetWritable, TweetWritable> {
    ...
    public void map(LongWritable key, Text value, Context context)
        ...
        try {
            ...
            context.getCounter(PipelineCounters.extractTweets.NUM_TWEETS_PROCESSED).increment(1);
            if (tweet.isEligibleForAnalysis) {
                context.write(tweet, tweet);
                context.getCounter(PipelineCounters.extractTweets.NUM_OUTPUTS_JSON_TO_TWEET_MAPPER).increment(1);
            }

        } catch (ParseException e) {
            context.getCounter(PipelineCounters.extractTweets.JSON_PARSE_EXCEPTION).increment(1);
        }
    }
}
